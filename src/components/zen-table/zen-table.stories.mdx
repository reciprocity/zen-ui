...
export const StoryWithControls = args => {
  ...
  function selectRow(row) {
    ...
    if (!row.tagName.endsWith('-ROW') || row.header) return;
  ...
  //
  function insertRow() {
    const parent = element(`sb-zen-table-row.selected`);
    if (!parent) {
  ...
  return html/*html*/ ` <style>
    ...
    <sb-zen-text bold class="mt-24">Zen-table rows editor</sb-zen-text>
