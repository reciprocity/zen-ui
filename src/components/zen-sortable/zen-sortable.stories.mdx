...
export const StorySortable = () => {
  return html/*html*/ ` <sb-zen-sortable id="sortable" style="width:230px;border:1px solid lightgray;">
      <sb-zen-sortable-item data-id="1">
        <sb-zen-text>Title</sb-zen-text>
        <sb-zen-toggle class="space-before"></sb-zen-toggle>
...
Slotted elements becomes draggable. It's advised to use `zen-sortable-items` for slotted elements. Each draggable item should have unique `data-id` attribute. On item drag&drop `drop` event is triggered. It contains list of `data-id`s that resembles new order.
...
<sb-docs-table docs={JSON.stringify(getComponentData('zen-sortable'))}></sb-docs-table>
