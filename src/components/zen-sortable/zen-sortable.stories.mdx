...
export const StoryWithControls = args => {
  waitForElement('#controls', element => {
    element.addEventListener('onChange', ev => {
...
Slotted content becomes draggable. On change position it returns an "onChange" custom event that contains list of "data-id" attributes value.
