...
- Dropdown ships in different sizes
- These sizes only matter if you have default items (simple text)
- Make sure you also set the same `size` value to zen-options!
...
- Set `size` to `custom` in order. It will make dropdown's height equal to height selected option.
